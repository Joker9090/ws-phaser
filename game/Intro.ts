  ...
  */
  ...
  create() {
    ...
    this.noswarText = this.add
      ...
      .setOrigin(0.5)
      .setDepth(99);
    this.studiosText = this.add
      .text(-(width + this.container.width + 10), height / 2, "studios", {
        fontFamily: "Arcade",
        fontSize: "30px",
        color: "#c3c5c3",
        shadow: {
    ...
    this.noswarLogo = this.add
      .image(0, 0, "logoNoswar")
      .setOrigin(0)
      .setScale(0.5)
      .setDepth(9);
    this.noswarLogo.setPosition(
      this.cameras.main.width - this.noswarLogo.width / 3.2,
      0
    );
    this.spaceText = this.add
      .text(width / 2, height / 2 + 200, "Press SPACE to continue")
      .setOrigin(0.5)
      .setAlpha(0);
    ...
    this.tweens.addCounter({
      ...
      onUpdate: (tween) => {
        if (this.noswarText) {
          const value = tween.getValue();
          this.studiosText?.setPosition(
            value,
            height / 2 + this.noswarText.height / 1.2
          );
        }
      },
    });
  ...
  update() {
    if (this.finishIntro) {
      ...
      if (this.noswarText)
        this.studiosText?.setPosition(
          this.cameras.main.width / 2,
          this.cameras.main.height / 2 + this.noswarText.height / 1.2
        );
      if (this.spaceText)
      ...
      if (this.cameras.main.width < this.cameras.main.height) {
        ...
        this.noswarLogo?.setScale(
          (0.5 * this.cameras.main.width) / this.cameras.main.height
        );
      } else if (this.cameras.main.width >= this.cameras.main.height) {
        this.container?.setScale(1);
        this.noswarLogo?.setScale(0.5);
      }
    }
  }
}
