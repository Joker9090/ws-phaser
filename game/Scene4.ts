...
const createPlatform = (group: Phaser.GameObjects.Group, spriteWidth: number, texture: string, dist: number) => {
  ...
  if (plataformas.indexOf(texture) != -1) {
    platform.setImmovable(true)
    platform.setSize(platform.displayWidth * 2, platform.displayHeigh - 50)
  }
  switch (true) {
    case (plataformas.indexOf(texture) != -1):
      platform.setDepth(2);
      break;
    case (texture == 'plateau'):
      platform.setDepth(1);
      break;
    default:
  }
...
class game extends Phaser.Scene {
  ...
  create() {
    ...
    if (this.player) {
      ...
      this.physics.add.overlap(this.player, this.groundGroup, (player, platform) => {
