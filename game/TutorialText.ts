...
export default class TutorialText extends Phaser.Scene {
  ...
  DisplayText(param: number) {
    ...
    if (param == 0) {
      ...
      this.tutorialTextBox?.setPosition(
        this.cameras.main.width / 2 - 350,
        this.cameras.main.height / 2 - 100
      );
      this.tutorialTextBox?.setTextBox(
        "Be careful with fireballs, they can kill you and you'll lose a life"
      );
    }
  }
  ...
  create() {
    ...
    /* TEXT BOX */
    this.tutorialTextBox = new TextBox(
      this,
      textPosX,
      textPosY,
      "textBox",
  ...
  update() {
    if (this.tutorialTextBox) {
      if (this.cameras.main.width < this.cameras.main.height) {
        this.tutorialTextBox.setScale(
          (0.8 * this.cameras.main.width) / this.cameras.main.height
        );
