...
class Boss extends Phaser.Physics.Arcade.Sprite {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, sprite: string, frame: number,life?: number) {
    ...
    if(this.body) {
      ...
      this.body.setSize(35, 85, true); // GOOOD!
  ...
  createAnims(scene: Phaser.Scene, sprite: string) {
    ...
    const skeletonIdleFrames = scene.anims.generateFrameNumbers(sprite,{start:0 , end: 6});
    //const skeletonWalkFrames = scene.anims.generateFrameNumbers("skeleton", {frames: [6,7,8,9,10,11]});
    const skeletonWalkFrames = scene.anims.generateFrameNumbers(sprite, {start: 28, end:34});
    const skeletonMoveFrames = scene.anims.generateFrameNumbers(sprite, { start:10 , end: 15 });
    const skeletonJumpFrames = scene.anims.generateFrameNumbers(sprite, {start: 14, end: 22});
    const skeletonDeadFrames = scene.anims.generateFrameNumbers(sprite,{start:41 , end: 46});
    const skeletonDmgFrames = scene.anims.generateFrameNumbers(sprite,{start:55 , end: 57});
    const skeletonDefFrames = scene.anims.generateFrameNumbers(sprite,{start:69 , end: 72});
    const skeletonAttackFrames = scene.anims.generateFrameNumbers(sprite,{start:83 , end: 86});
    const skeletonMagicAttackOneFrames = scene.anims.generateFrameNumbers(sprite,{start: 97, end: 110});
    ...
    const skeletonJumpFramesConfig = {
      key: `${sprite}Jump`,
    ...
    const skeletonMagicAttackOneFramesConfig = {
      key: `${sprite}MagicOne`,
      frames: skeletonMagicAttackOneFrames,
      frameRate: 10,
      repeat: 0,
    }
    ...
    scene.anims.create(skeletonMagicAttackOneFramesConfig);
    scene.anims.create(skeletonJumpFramesConfig);
