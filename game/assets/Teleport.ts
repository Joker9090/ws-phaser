...
class Teleport extends Phaser.GameObjects.Sprite {
    ...
    trigger() {
        if (this.config.sameScene) {
            ...
            this.scene.changeScene({...this.config.otherSceneConf, lifes: this.scene.lifes ?? 3, collectedItems: this.scene.collectedItems, seconds: this.scene.seconds, minutes: this.scene.minutes}) // data
        }
