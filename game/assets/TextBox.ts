...
class TextBox extends Phaser.GameObjects.Container {
    ...
    constructor(scene: Phaser.Scene, x: number, y: number, texture: string | Phaser.Textures.Texture, widthText: number) {
        super(scene);
        this.scene = scene;
        ...
        this.tutorialText = this.scene.add.text(
            ...
            0,
            '',
            { fontFamily: 'Arcade', fontSize: '30px', wordWrap: { width: widthText, useAdvancedWrap: true } }
        )
            .setOrigin(.5)
            .setScrollFactor(0)
            .setDepth(200);
        ...
        this.rectanguleText = this.scene.add.sprite(0, 0, texture)
            .setScrollFactor(0)
            .setDepth(190)
