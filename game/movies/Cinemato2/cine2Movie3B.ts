...
class cine2Movie3B {
  ...
  playCine(this: cine2Movie3B) {
    ...
    const part1 = (job: TickerJob) => {
      ...
      const weldingEase = (t: number) => {
        const cycleTime = 5; // Duración total del ciclo de 6 segundos
  ...
  update(this: cine2Movie3B, time: number, delta: number) {
    if (this.dialogue) this.dialogue.update();
    if (this.nextCine) this.cine.scene.restart({ keyname: "cine_2_movie_3" });
  }
}

export default cine2Movie3B;
