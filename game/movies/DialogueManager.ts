...
class DialogueManager {
  ...
  constructor(
    ...
    // if (this.config[this.textCounter].position && this.config[this.textCounter].width)
    this.container
  ...
  styleContainer(position: {x: number, y:number}, width: number, text: string){
    const height = 100
    this.textDisplayed = this.scene.add.text(40, 40, "", {
      color: "black",
      fontSize: 20,
    ...
    this.container?.setPosition(position.x, position.y).setSize(width, height)
